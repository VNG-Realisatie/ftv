// Gevendorde JSON-LD-contexts als importeerbare module (node- én browser-ESM;
// geen fetch en geen JSON-import-attributes nodig).
//
// GEGENEREERD uit de ongewijzigde bronbestanden in deze map:
//   odrl.jsonld     — http://www.w3.org/ns/odrl.jsonld (W3C)
//   rightsml.jsonld — https://iptc.org/std/RightsML/odrl-profile/rightsml.jsonld (IPTC)
// Zie README.md hiernaast voor bron, datum en licentie. Niet met de hand
// bewerken; hergenereer met:  node viewer/vendor/contexts/generate.mjs

// URL -> het `@context`-object van de gevendorde context.
export const KNOWN_CONTEXTS = {
  ...
  "https://iptc.org/std/RightsML/odrl-profile/rightsml.jsonld": {
    "rml": "http://iptc.org/std/RightsML/2011-10-07/",
    "compensationValue": "rml:compensationValue",
    "odrl": "http://www.w3.org/ns/odrl/2/",
    "acceptTracking": "odrl:acceptTracking",
    "aggregate": "odrl:aggregate",
    "annotate": "odrl:annotate",
...
// Zoek een gevendorde context op URL. Matcht schema-ongevoelig (http/https)
// en negeert een trailing slash, zodat beide gangbare schrijfwijzen werken.
export function knownContext(url) {
  if (typeof url !== 'string') return null;
  const key = url.replace(/^https?:\/\//, '').replace(/\/$/, '');
  for (const [u, ctx] of Object.entries(KNOWN_CONTEXTS)) {
    if (u.replace(/^https?:\/\//, '').replace(/\/$/, '') === key) return ctx;
  }
  return null;
}
